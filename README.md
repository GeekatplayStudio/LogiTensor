...
- **Data & Text**: **Filter** (passes a value through only if it includes/excludes a search term, case-sensitive optional), Text Transform (uppercase/lowercase/trim/length/reverse), Text Replace, **Split** / **Join**, **Substring**, **Template** (fills `{a}`/`{b}` placeholders from auto-growing inputs), **JSON Parse** / **Stringify**, **To Number** / **To String** / **To Boolean**, **Regex Match**.
- **Lists**: **Append** (accumulates values into an array, with reset), **Length**, **Get** (index, negative counts from the end), **Stats** (sum/avg/min/max/count), **Sort** (numeric or lexical, asc/desc), **Slice**, **Contains**.
