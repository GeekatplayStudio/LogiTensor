...
flowchart LR
    ...
    subgraph NewSurfaces["Build & Verify Faster"]
        ...
        Code["📄 Live Code Panel<br/>10 languages, one graph"]:::new
...
- **Future improvements:** real sandboxing (subprocess isolation or WASM) for user-authored Python before any multi-user or hosted deployment; a persistence layer if collaboration or workflow history becomes a goal; extending parity testing to the trigger-driven path; and widening the code-generation panel's native emitters (currently TypeScript/JavaScript and Python) to more languages as demand shows up, rather than leaning further on the line-adapter approach used for C/C++/Go/Rust/Ruby/PHP today.
...
- A collapsible, resizable right-hand panel renders the current canvas graph as real source code, live, with a language dropdown: **TypeScript, JavaScript, Python, C, C++, Go, Rust, Ruby, PHP, MicroPython**. It follows the canvas continuously — building, editing, or **loading a saved flow** repopulates it immediately.
- **Build Logic (code → graph)**: click the pencil to detach the panel, then paste, hand-edit, or **drag a source file straight onto the panel** (`.ts`, `.py`, `.ino`, `.rs`, …) — then **Build Logic** to rebuild the canvas from it. Because deterministic parsers for ten languages aren't practical, it routes the code through the same local-Ollama-plus-schema-validation path as the natural-language builder — with a code-analysis system prompt that demands a *detailed* flow (one node per meaningful step, every literal preserved, a scripting-node fallback that keeps the original code text when no catalog node fits). Nothing reaches the canvas until every node type, port, and edge is verified against `NODE_DEFINITIONS`. Edit mode has its own **model picker** and **replace/add-to-board toggle**, same as the NL bar. See [`docs/BUILD_LOGIC.md`](./docs/BUILD_LOGIC.md) for the full pipeline.
...
- TypeScript/JavaScript and Python are **native emitters** — one small per-node-type contract (`setup`/`loop`/`outputs`) shared across ~46 node types, so trigger chains become real `if`/loop/function statements and data wiring becomes threaded expressions, not templated strings. C/C++/Go/Rust/Ruby/PHP are **derived** from the JavaScript emission — and **MicroPython** from the Python emission — through a deterministic line-adapter (rule table + honest `TODO(port by hand)` bailouts for constructs that don't map cleanly) — the same approach Geekatplay Studio's ASH-Mesh Device Logic Studio uses, extended from its original 4-language, 2-native-emitter design.
...
- **Drag and drop files in**: drop a saved flow `.json` anywhere on the canvas to open it (shape-checked first, so a stray file can never wipe the board), or drop a source file (`.ts`, `.py`, `.ino`, …) on the code panel to load it into the editor ready for **Build Logic**. A flow JSON dropped on the code panel is recognized and loaded onto the board instead. Files dropped anywhere else are swallowed rather than navigating the browser away from the session.
...
- [`demo/`](./demo/README.md) — demo gallery: deliberately messy TypeScript files to paste into Build Logic (OAuth access checks, ESP32 signal triage), plus pre-verified workflow JSONs to load directly (WiFi/BLE signal monitor, digit-recognizer mini network, image-preview pipeline).
